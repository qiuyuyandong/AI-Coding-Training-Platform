...
- Final review-work QA hash deviation: a later final review-work QA lane once mistakenly invoked `Get-FileHash` on the default `training-platform.sqlite` during its initial state capture and also opened the file once via `Get-Item`, violating the no-open/hash review process rule; the hash was discarded and never used as evidence, no write occurred, and default DB `Length` 73728 / `LastWriteTimeUtc` 2026-07-13T17:49:36.9126118Z remained unchanged
...
- Final review-work QA hash deviation: a later final review-work QA lane once mistakenly invoked `Get-FileHash` on the default `training-platform.sqlite` during its initial state capture and also opened the file once via `Get-Item`, violating the no-open/hash review process rule; the hash was discarded immediately and was never recorded as evidence or used as a gate signal, no write occurred, and default DB `Length` 73728 / `LastWriteTimeUtc` 2026-07-13T17:49:36.9126118Z remained unchanged; full record in `work/reports/phase-0d-engineering-gates.md` Post-Task 6 review note